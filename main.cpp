...
public:
  ...
  std::string getSettingsPath() {
    ...
    if (!localAppData) {
      std::cerr << "Could not get LOCALAPPDATA environment variable."
                << std::endl;
      std::exit(1);
    }
    ...
    if (!std::filesystem::exists(path)) {
      std::cerr << "settings.json does not exist. Make sure Windows Terminal "
                   "is installed."
                << std::endl;
      std::exit(1);
    }
...
public:
  WTACommandManager() {
    ...
    commands["colorscheme"] = [this](const std::vector<std::string> &args) {
      colorSchemeCommand(args);
    };
    commands["font"] = [this](const std::vector<std::string> &args) {
      fontCommand(args);
    };
    commands["createprofile"] = [this](const std::vector<std::string> &args) {
      createProfileCommand(args);
    };
    commands["elevate"] = [this](const std::vector<std::string> &args) {
      elevateCommand(args);
    };
  }
  ...
  void colorSchemeCommand(const std::vector<std::string> &args) {
    if (args.size() < 1 || args.size() > 2) {
      std::cerr << "Usage: wta colorscheme <schemeName> [profileName]"
                << std::endl;
      return;
    }
  ...
  void fontCommand(const std::vector<std::string> &args) {
    if (args.empty() || args.size() > 3) {
      std::cerr << "Usage: wta font <fontName> [fontSize] [profileName]"
                << std::endl;
      return;
    }
  ...
  void createProfileCommand(const std::vector<std::string> &args) {
    ...
    json newProfile = {{"name", profileName},
                       {"commandline", "cmd.exe"},
  ...
  void elevateCommand(const std::vector<std::string> &args) {
    if (args.size() < 1) {
      std::cerr << "Usage: wta elevate <true | false> [profileName]"
                << std::endl;
      return;
    }
    ...
    if (elevateOption != "true" && elevateOption != "false") {
      std::cerr << "Invalid option for elevate: " << elevateOption
